...
**Emergency stop is a guarded control.** It sits apart on the right, and it is a
**press-and-hold** — roughly a second, with a ring that fills — rather than a button behind a
confirmation dialog. The reasoning: an emergency stop needs to be fast, and a modal is not
fast; but an accidental emergency stop on the wrong Drone is worse than a slow one. Physical
guarded switches solve this exact problem with deliberate effort rather than with a
