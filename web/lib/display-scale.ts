 ...
 * and a lit classroom with the board projected at the front is not the size a laptop on
 * a desk is designed for.
 *
...
export const DISPLAY_SCALE_KEY = 'techtechflight:display-scale'
...
export function readDisplayScale(): boolean {
  ...
  try {
    return window.localStorage.getItem(DISPLAY_SCALE_KEY) === 'large'
  } catch {
...
export function applyDisplayScale(large: boolean): void {
  ...
  try {
    window.localStorage.setItem(DISPLAY_SCALE_KEY, large ? 'large' : 'standard')
  } catch {
