 ...
 *
 * Carries the tilde for the same reason an estimated battery reading does: the number is
 * inferred rather than measured, and the board never lets an inference dress up as a
...
export function formatTimeToReady(ms: number): string {
  ...
  return remainder === 0 ? `Ready in ~${hours} hr` : `Ready in ~${hours} hr ${remainder} min`
}
