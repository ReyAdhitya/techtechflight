 ...
 * Command that makes a Drone do more than it is already doing, and taking off is
 * deliberately not one — a Teacher does not launch a Student's Drone from across the room.
 * That makes the whole surface fail safe by construction: the worst outcome of a mistaken
 * Command is an aircraft that comes down when it did not need to.
 */

export type CommandKind = 'land' | 'hold' | 'auto-land' | 'emergency-stop'
